...
int main(int argc, char **argv) {
  ...
  //@@ Allocate GPU memory
  cudaMalloc((void **) &dev_in, numInputElements * sizeof(float));
  cudaMalloc((void **) &dev_out, numOutputElements * sizeof(float));
  //@@ Copy input memory to the GPU
  cudaMemcpy(dev_in, hostInput, numInputElements * sizeof(float), cudaMemcpyHostToDevice);
  //@@ Initialize the grid and block dimensions here
  dim3 DimGrid(ceil((float) numInputElements / BLOCK_SIZE), 1, 1);
  dim3 DimBlock(BLOCK_SIZE, 1, 1);
  //@@ Launch the GPU Kernel and perform CUDA computation
  total<<<DimGrid, DimBlock>>>(dev_in, dev_out, numInputElements);
  cudaDeviceSynchronize();  
  //@@ Copy the GPU output memory back to the CPU
  cudaMemcpy(hostOutput, dev_out, numOutputElements * sizeof(float), cudaMemcpyDeviceToHost);
  /********************************************************************
