    ...
    */
    ...
    for (int i = 0; i < ceil((float) unrolled / TILE_WIDTH); i++) {
        ...
        if (col < hw && uv < unrolled) {
            ...
            tileInput[ty][tx] = (h < Height && w < Width) ? __float2half(in_4d(b, c, h, w)) : __float2half(0);
        } else {
            tileInput[ty][tx] = __float2half(0);
        }
        int j = i * TILE_WIDTH + tx;
        if (row < Map_out && j < unrolled) {
            tileMask[ty][tx] = __float2half(mask[unrolled * row + j]);
        } else {
    ...
    if (row < Map_out && col < hw) {
        output[Map_out * hw * b + hw * row + col] = tileOutput[ty][tx];
    }
    #undef in_4d
}
