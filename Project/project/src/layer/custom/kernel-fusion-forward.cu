                                  ...
                                  int Batch, int Map_out, int Channel, int Height, int Width, int K)
{
    ...
    */
    ...
    for (int i = 0; i < ceil((float) unrolled / TILE_WIDTH); i++) {
        if (unrolled <= TILE_WIDTH * i + ty || hw <= col) {
            tileInput[ty][tx] = 0;
        } else {
            const int tmp = TILE_WIDTH * i + ty;
            const int p = (tmp - tmp / (K * K) * K * K) / K;
            const int q = (tmp - tmp / (K * K) * K * K) % K;
            if (col / Width_out + p < Height && col % Width_out + q < Width) {
                tileInput[ty][tx] = in_4d(b, (TILE_WIDTH * i + ty) / K / K, col / Width_out + p, col % Width_out + q);
            } else {
                tileInput[ty][tx] = 0;
            }
        }
